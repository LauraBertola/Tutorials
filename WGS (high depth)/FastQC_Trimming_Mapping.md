...
```
/softwares/FastQC/fastqc ~/Tutorial_WGS_HD/input_files/* -o ~/Tutorial_WGS_HD/output_files/
```

On the screen, you'll see the progress of your FastQC run. 
![fastqc_progress](Images/fastqc_progress.png)

Note that we use *, which is a wildcard. It means *any file in the directory input_files/*. You can use wildcards also more specifically, for example `*.fq.gz`, which means all fq.gz files. Or `*R1*`, which is all files with R1 anywhere in their name, which comes in handy if you only want to process forward reads, for example.
...
```
conda create -n multiqc --file ~/admin/multiqc_env.txt
```
```
